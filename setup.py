...
if platform == platform_detect.RASPBERRY_PI:
    ...
    if pi_version == 1:
        ...
        raise RuntimeError('Detected Pi version that has no appropriate driver '
                           'available.')
    ...
    print 'Could not detect if running on the Raspberry Pi or Beaglebone Black.' \
          'If this failure is unexpected, you can run again with --force-pi or ' \
          '--force-bbb parameter to force using the Raspberry Pi or Beaglebone ' \
          'Black respectively.'
    sys.exit(1)
...
setup(name              = 'Adafruit_DHT',
      ...
      description       = 'Library to get readings from the DHT11, DHT22, and '
                          'AM2302 humidity and temperature sensors on a '
